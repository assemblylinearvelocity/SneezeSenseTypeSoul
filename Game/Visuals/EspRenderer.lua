...
local function GetBoundingBox(character)
    ...
    return Vector2.new(math.round(minX), math.round(minY)), Vector2.new(math.round(maxX), math.round(maxY))
end
...
function EspRenderer.new(player)
    ...
    healthText.Visible = false ; healthText.Size = 10 ; healthText.Center = true
    healthText.Outline = true  ; healthText.Color = Color3.fromRGB(255,255,255)
    self.healthText = healthText
    local nameText = Drawing.new("Text")
    nameText.Visible = false ; nameText.Size = 16 ; nameText.Center = true
    nameText.Outline = true  ; nameText.Color = Color3.fromRGB(255,255,255)
...
function EspRenderer:UpdateBox(min, max, color)
    local o, i = 1, 1
    local oTL = Vector2.new(min.X-o, min.Y-o) ; local oTR = Vector2.new(max.X+o, min.Y-o)
    local oBL = Vector2.new(min.X-o, max.Y+o) ; local oBR = Vector2.new(max.X+o, max.Y+o)
    ...
    local mTL=Vector2.new(min.X,min.Y) ; local mTR=Vector2.new(max.X,min.Y)
    local mBL=Vector2.new(min.X,max.Y) ; local mBR=Vector2.new(max.X,max.Y)
    ...
    for _,l in pairs(self.box.main) do l.Color=color or Color3.fromRGB(255,255,255) ; l.Visible=true end
    local iTL=Vector2.new(min.X+i,min.Y+i) ; local iTR=Vector2.new(max.X-i,min.Y+i)
    local iBL=Vector2.new(min.X+i,max.Y-i) ; local iBR=Vector2.new(max.X-i,max.Y-i)
...
function EspRenderer:HideHealthBar()
    self.healthBar.outlineLeft.Visible=false ; self.healthBar.outlineRight.Visible=false
    self.healthBar.outlineTop.Visible=false  ; self.healthBar.outlineBottom.Visible=false
    self.healthBar.fill.Visible=false        ; self.healthText.Visible=false
end
...
function EspRenderer:UpdateHealthBar(min, max, character, showText)
    ...
    self.healthBar.fill.Color=HpToColor(pct) ; self.healthBar.fill.Visible=pct>0
    if showText then
        ...
        self.healthText.Center=true ; self.healthText.Visible=true
    else
...
function EspRenderer:HideBox()
    ...
    self:HideHealthBar() ; self:HideName()
end
...
function EspRenderer:Update(character, flags)
    ...
    if boxOn and character and character:FindFirstChild("HumanoidRootPart") and character:FindFirstChildOfClass("Humanoid") then
        ...
        if min and max then
            ...
            if hpBarOn then self:UpdateHealthBar(min, max, character, flags["Health Text"]) else self:HideHealthBar() end
            if flags["Name ESP"] then self:UpdateName(min, max, flags["Name Mode"] or "Both") else self:HideName() end
        else self:HideBox() end
    else self:HideBox() end
end

function EspRenderer:Destroy()
    for _,set in pairs(self.box) do for _,l in pairs(set) do l:Remove() end end
    self.healthBar.outlineLeft:Remove() ; self.healthBar.outlineRight:Remove()
    self.healthBar.outlineTop:Remove()  ; self.healthBar.outlineBottom:Remove()
    self.healthBar.fill:Remove() ; self.healthText:Remove() ; self.nameText:Remove()
end
